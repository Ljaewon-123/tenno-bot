...
export const partyExpiresAt = (party: Party) =>
  party.createdAt.add(PARTY_EXPIRE_HOURS, 'hour');
...
export const partyMessage = (party: Party) => {
  ...
  if (open) {
    embed.addFields({
      name: 'Closes',
      value: `<t:${partyExpiresAt(party).unix()}:R>`,
    });
  } else {
  ...
  return {
    embeds: [embed],
    components: open ? [buttons(party.id)] : [],
  };
};
